...
def _build_maker_tools(jail: Jail, budgets: Budgets) -> list:
    """Deliberately conservative tool surface for Phase 1: read/list/write
    plus a scoped pytest runner. `Jail` also supports apply_patch/delete/move
    and `run_command` supports more binaries -- not yet exposed to the maker
    here; see PHASED_PLAN.md "Deviations"."""
    ...
    return [read_file, list_dir, write_file, run_pytest]
...
def _maker_task_text(state: CodingLoopState) -> str:
    ...
    return (
        ...
        "list directories, write files, and run pytest to check your work."
    )
...
def self_check_node(state: CodingLoopState) -> dict:
    ...
    ruff_result = run_command(
        "ruff", ["check", ".", "--select", "E9,F"], cwd=worktree_dir, timeout_s=timeout
    )
...
def bdd_gate_node(state: CodingLoopState) -> dict:
    ...
    result = run_command(
        "pytest",
        ["-q", *scope_dirs, "--json-report", f"--json-report-file={report_file.name}"],
        cwd=worktree_dir,
        timeout_s=timeout,
    )
    summary = _read_json_report(report_file)
